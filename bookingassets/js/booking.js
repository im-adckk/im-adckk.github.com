        ...
        if (data) {
            ...
            if (!warningContainer.querySelector('#dayDuplicateWarningAlert')) {
                warningContainer.innerHTML = `
                    <div id="dayDuplicateWarningAlert" class="alert mb-4" data-variant="warning">
                        <i data-lucide="triangle-alert" class="w-4 h-4"></i>
                        <span class="text-sm">
                            <strong>Warning:</strong> You already have a booking on this date
                            (Session: ${data.session_time} - ${data.session_slot}).
                            You can only have ONE booking per day. Please cancel your existing booking first.
                        </span>
                    </div>
                `;
                refreshIcons();
